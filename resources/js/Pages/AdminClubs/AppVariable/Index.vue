...
const save = () => {
    formSendRef.value?.validate().then(({ valid: isValid }) => {
        if (!isValid) {
            ...
            if (form.id) {
                form.put(route("app-variables.update", form.id), {
                    onSuccess: () => {
                        customToastSwal({
                ...
                form.post(route("app-variables.store"), {
                    onSuccess: () => {
                        customToastSwal({
                            title: page.props.flash.success || "",
                            icon: "success",
...
const destroy = (data: any) => {
    customConfirmSwal({
        ...
        if (result.isConfirmed) {
            form.delete(route("app-variables.destroy", data.id), {
                onSuccess: () => {
                    customToastSwal({
                        title: page.props.flash.success || "",
                        icon: "success",
...
const fetchItems = async () => {
    ...
    router.get(route("app-variables.index"), params, {
        preserveState: true,
        replace: true,
        onSuccess: (page) => {
...
<template>
    ...
    <AppLayout>
        <template #header> Variables de App Móvil</template>
        <template #options>
            <BaseButton
                variant="elevated"
                :icon-only="false"
                @click="create()"
                action="add"
                v-if="can.includes('app-variables.store')"
            />
        </template>
                    ...
                    >
                        ...
                        <template #item.actions="{ item }">
                            <BaseButton
                                action="edit"
                                @click="edit(item)"
                                v-if="can.includes('app-variables.update')"
                            />
                            <BaseButton
                                @click="destroy(item)"
                                action="delete"
                                v-if="can.includes('app-variables.destroy')"
                            />
                        </template>
