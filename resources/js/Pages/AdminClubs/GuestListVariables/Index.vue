...
interface Props {
    guestListVariables?: any;
}

const props = withDefaults(defineProps<Props>(), {
    guestListVariables: null,
});

interface guestListVariable {
    id: number | null;
    code: string;
    name: string;
    value: string;
...
const form = useForm<guestListVariable>({
    id: null,
    code: '',
    name: '',
    value: '',
...
const save = () => {
    formSendRef.value?.validate().then(({ valid: isValid }) => {
        ...
        if (!isValid) {
            ...
            if (form.id) {
                form.put(route("guest-list-variables.update", form.id), {
                    onSuccess: () => {
                        customToastSwal({
                ...
                form.post(route("guest-list-variables.store"), {
                    onSuccess: () => {
                        customToastSwal({
                            title: page.props.flash.success || "",
                            icon: "success",
...
const destroy = (data: any) => {
    customConfirmSwal({
        ...
        if (result.isConfirmed) {
            form.delete(route("guest-list-variables.destroy", data.id), {
                onSuccess: () => {
                    customToastSwal({
                        title: page.props.flash.success || "",
                        icon: "success",
...
const prefix = "guestListVariables";
...
const fetchItems = async () => {
    ...
    router.get(route("guest-list-variables.index"), params, {
        preserveState: true,
        replace: true,
        onSuccess: (page) => {
...
<template>
    ...
    <AppLayout>
        <template #header> Configuración de Listas de Invitados </template>
        <template #options>
            <BaseButton
                variant="elevated"
                :icon-only="false"
                @click="create()"
                action="add"
                v-if="can.includes('guest-list-variables.store')"
            />
        </template>
                    ...
                    >
                        ...
                        <template #item.actions="{ item }">
                            <BaseButton
                                action="edit"
                                @click="edit(item)"
                                v-if="can.includes('guest-list-variables.update')"
                            />
                            <BaseButton
                                @click="destroy(item)"
                                action="delete"
                                v-if="can.includes('guest-list-variables.destroy')"
                            />
                        </template>
