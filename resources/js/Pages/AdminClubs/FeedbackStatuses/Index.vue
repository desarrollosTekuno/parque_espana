...
const form = useForm<FeedbackStatus>({
    id: null,
    name: "",
    code: "",
    color: "#6B7280",
...
const items = ref(props.statuses?.data ?? []);
const total = ref(props.statuses?.total ?? 0);
const loading = ref(false);
const search = ref("");

const options = ref({
    page: props.statuses?.current_page ?? 1,
    itemsPerPage: props.statuses?.per_page ?? 10,
...
const fetchItems = async () => {
    ...
    router.get(
        ...
        },
        {
            ...
            replace: true,
            onSuccess: (page) => {
                const statuses = page.props.statuses as any;
                items.value = statuses?.data ?? [];
                total.value = statuses?.total ?? 0;
                loading.value = false;
            },
            onError: () => {
                loading.value = false;
...
const save = () => {
    formSendRef.value?.validate().then(({ valid }: any) => {
        ...
        if (form.id) {
            form.put(route("feedback-statuses.update", form.id), {
                preserveScroll: true,
                onSuccess: () => {
        ...
        form.post(route("feedback-statuses.store"), {
            preserveScroll: true,
            onSuccess: () => {
                customToastSwal({ title: page.props.flash.success || "", icon: "success" });
                close();
...
<template>
    <Head title="Estatus de feedback" />
                    ...
                    >
                        ...
                        <template #item.color="{ item }">
                            <div class="d-flex align-center ga-2">
                                <span
                                    style="width: 14px; height: 14px; border-radius: 9999px; display: inline-block"
                        ...
                        <template #item.actions="{ item }">
                            <BaseButton
                                v-if="can.includes('feedback-statuses.update')"
                                action="edit"
                                @click="edit(item)"
                            />
