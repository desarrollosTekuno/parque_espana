...
const recipientsPagesCount = computed(() => {
    const totalValue = filteredRecipients.value.length;
    return Math.max(1, Math.ceil(totalValue / recipientsPerPage));
});
...
const setDefaultSmtpIfSingle = () => {
    if (form.smtp_config_id || smtpOptions.value.length === 0) {
        return;
    }
