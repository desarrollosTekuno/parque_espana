...
return new class extends Migration {
    public function up(): void
    {
        Schema::create('guest_lists.cafeteria_visits', function (Blueprint $table) {
            ...
            $table->string('document_type', 50);   // INE, pasaporte, licencia, credencial_trabajo, otro
            $table->string('document_number', 100);
            $table->timestamp('entered_at');
            $table->timestamp('expires_at');
            $table->decimal('min_consumption', 12, 2); // capturado al momento de ingreso desde variable
            $table->decimal('consumption_amount', 12, 2)->nullable();
            $table->boolean('access_fee_waived')->nullable();     // null hasta checkout
            $table->decimal('access_fee_charged', 12, 2)->nullable(); // monto cobrado si no aplica exención
